...
//! itself, and the listener's own log opened where it wants it. Not a test
//! target — a bare `common.rs` here would be compiled as one.
...
const LONGEST_TEST_NAME: &str = "a_launcher_signed_by_a_stranger_is_never_started";
...
/// The listener's log, pointed where testkit keeps this run's copy.
pub fn appLog() -> Log {
    Log::open(Some(appLogPath()))
}
